...
for k = 1:1
    ...
    %for j = 1:length(h2_vec)
		...
		% Generate the lambda
		lambda = linspace(-lambda_max(j), lambda_max(j), 100);
		...
		for a = 1:length(h_1)
			subplot(3, 3, a);
			line_curr = plot(lambda, tsls_vec(a, :), '-r', lambda, post_haus_vec(a, :), '-b', 'LineWidth', 2);
			...
			if a == length(h_1) - 2
				...
				hL = legend(line_curr, {'TSLS T-Test', 'Post-Hausman T-Test'}, ...
		...
		name = [name, '$\alpha = ', num2str(alpha), ...
			'$, $\beta_1 = ', num2str(beta_1), '$, $\beta^*_2 = ', num2str(beta2_optim_loc), ...
			'$, $ h_2 = ', num2str(h_2), '$'];
		...
		% Make a horizontal legend and display it at the bottom of the screen
		legend boxoff;
		newPosition = [0.5, 0.019, 0.075, 0.04];
        ...
        % Output the figure to an .m file for conversion later
        saveas(gcf, ['PowerCurves-twosided-h2-', num2str(j), '.m']); 
