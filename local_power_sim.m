...
mkdir('/users/nriabov/McCloskey2014-Sims/Hausman/beta1search/');
cd '/users/nriabov/McCloskey2014-Sims/Hausman/beta1search/'
...
%         alpha_vec = zeros(1:length(beta2_vec));
%         alpha_adj = 1;
%         l = 1;
%         while (alpha_adj > 0.046  || alpha_adj < 0.04) && l <= length(beta2_vec)
%             beta_2 = beta2_vec(l);
%             % Precompute alpha_adj
%             alpha_adj = level_adjust_mod(alpha, beta_1, beta_2, h_2, R, g, alpha_g, psi_up_1, psi_vp_1, psi_uvp_1);
%             alpha_vec(l) = alpha_adj;
%             l = l + 1;
%         end

        alpha_vec = level_adjust_mod(alpha, beta_1, beta2_vec, h_2, R, g, alpha_g, psi_up_1, psi_vp_1, psi_uvp_1);
        % Find the alpha_vec closest to 0.045 and use it.
        ...
        % Generate the lambda
        lambda = linspace(0, lambda_max(j), 100);

        % Make matrices to store results
        tsls_vec = zeros(num_h1, length(lambda));
        post_haus_vec = zeros(num_h1, length(lambda));
        ...
        for i = 1:length(h_1)
            [ tsls_pow, post_haus_pow ] = local_power_mod_2(R, alpha, beta_1, beta2_optim_loc, h_1(i), h_2, lambda, alpha_adj);
            tsls_vec(i,:) = tsls_pow;
        ...
        for a = 1:length(h_1)
            ...
            title(title_vec, 'Interpreter', 'latex', 'Fontsize', 14)
            if a == length(h_1)
        ...
        text(0.5, 1, name ,'HorizontalAlignment', ...
            'center','VerticalAlignment', 'top', 'Interpreter', 'latex', 'Fontsize', 14)
