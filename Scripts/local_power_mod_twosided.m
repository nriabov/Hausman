function [ tsls_pow, post_haus_pow ] = local_power_mod_twosided(R, alpha, beta_1, beta_2, h_1, h_2, lambda, alpha_adj)
...
eta_lhs = Eta_star_mod_twosided(eta_1, eta_2, eta_3, beta_1, lambda, h_2);

% Calculate alpha_adj
if nargin < 8
    alpha_adj = level_adjust_twosided(alpha, beta_1, beta_2, h_2, R, g, alpha_g);
end
...
eta_quant_mat = Eta_quantile_gen_vec_twosided(R, h1_loc_val, h_2, alpha_adj, beta_1);
...
parfor xi_ind = 1:length(xi_2)
    ...
    % boundaries
    eta_quant_rel = eta_quant_mat(xi_low_indices(xi_ind): xi_hi_indices(xi_ind));
