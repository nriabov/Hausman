function [ alpha_adj ] = level_adjust_twosided( alpha, beta_1, beta_2, h_2, R, g, alpha_g)
...
tic
if (beta_1 <= 0.9 && alpha_g > 0.001)
   if h_2 <= 0.001
       h1_cand = -5000:1000:(10/h_2);
   elseif h_2 > 0.001 && h_2 <= 0.75 % If h_2 is small, start with a wide range of potential h1 values
       h1_cand = -200:10:(20/h_2);
...
for h1_ind = 1:length(h1_vals)
    ...
    % The next bit depends on the size of the beta_2 matrix (FINISH ME!!!)
    for l = 1:length(beta_2)
        % Generate the grid of h1_local for the entire domain between
        % I_lo_min and I_hi_max
        
        if l == 1
            h1_loc_val = linspace(I_lo_min(l), I_hi_max(l), 40);
            dist = h1_loc_val(2) - h1_loc_val(1);
        else
            h1_loc_val = I_lo_min(l):dist:I_hi_max(l); 
        end
        ...
        [~, xi_lo] = min(abs(bsxfun(@minus, I_lo(l,:)', h1_loc_val)),[],2);
        [~, xi_hi] = min(abs(bsxfun(@minus, I_hi(l,:)', h1_loc_val)),[],2);
        xi_lo = cast(xi_lo, 'uint32');
        xi_hi = cast(xi_hi, 'uint32');
        ...
        while probvals > alpha && j <= g % Stop if either probvals is <= alpha or you reach the end of alpha_vec
            ...
            parfor xi_ind = 1:length(xi_2)
                ...
                sup_eta(xi_ind) = max(eta_quant_loc(xi_lo(xi_ind): xi_hi(xi_ind)));
            end

            % Calculate the probability values
            probvals = sum(abs(eta_star) > sup_eta')/length(eta_star);
