function [ alpha_adj ] = level_adjust_mod( alpha, beta_1, beta_2, h_2, R, g, alpha_g, psi_up_1, psi_vp_1, psi_uvp_1)
...
if beta_1 < 0 || beta_1 > 1 || logical(sum(beta_2 < 0)) || logical(sum(beta_2 > 1))
...
if (beta_1 <= 0.9 && alpha_g > 0.001)
   ...
   h1_vals = linspace(h1_cand(min_ind), h1_cand(max_ind), 40);
...
for h1_ind = 1:length(h1_vals)
    ...
    for l = 1:length(beta_2)
        ...
        [~, xi_low_indices] = min(abs(bsxfun(@minus, I_lo(l,:)', h1_loc_val)),[],2);
